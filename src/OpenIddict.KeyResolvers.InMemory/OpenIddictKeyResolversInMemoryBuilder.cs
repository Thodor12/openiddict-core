...
/// </summary>
public class OpenIddictKeyResolversInMemoryBuilder
{
    /// <summary>
    /// Initializes a new instance of <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.
    /// </summary>
    /// <param name="services">The services collection.</param>
    public OpenIddictKeyResolversInMemoryBuilder(IServiceCollection services)
    ...
    /// <summary>
    /// Amends the default OpenIddict in memory key resolver configuration.
    /// </summary>
    /// <param name="configuration">The delegate used to configure the OpenIddict options.</param>
    /// <remarks>This extension can be safely called multiple times.</remarks>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder Configure(Action<OpenIddictKeyResolversInMemoryOptions> configuration)
    {
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCredentials(EncryptingCredentials credentials)
    {
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionKey(SecurityKey key)
    {
        if (key is null)
        {
    ...
    /// </summary>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddDevelopmentEncryptionCertificate()
    ...
    public OpenIddictKeyResolversInMemoryBuilder AddDevelopmentEncryptionCertificate(X500DistinguishedName subject)
    {
        if (subject is null)
        {
    ...
    /// </summary>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEphemeralEncryptionKey()
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEphemeralEncryptionKey(string algorithm)
    {
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(X509Certificate2 certificate)
    {
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(Assembly assembly, string resource, string? password)
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(
    ...
    /// <param name="password">The password used to open the certificate.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(Stream stream, string? password)
#if SUPPORTS_EPHEMERAL_KEY_SETS
                // Note: ephemeral key sets are currently not supported on macOS.
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The X.509 certificate is attached to the server options.")]
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(Stream stream, string? password, X509KeyStorageFlags flags)
    {
        if (stream is null)
        {
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(string thumbprint)
    {
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEncryptionCertificate(string thumbprint, StoreName name, StoreLocation location)
    {
        if (string.IsNullOrEmpty(thumbprint))
        {
            throw new ArgumentException(SR.GetResourceString(SR.ID0065), nameof(thumbprint));
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCredentials(SigningCredentials credentials)
    {
        if (credentials is null)
        {
            throw new ArgumentNullException(nameof(credentials));
    ...
    /// <param name="key">The security key.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningKey(SecurityKey key)
    {
        if (key is null)
        {
            throw new ArgumentNullException(nameof(key));
    ...
    /// </summary>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddDevelopmentSigningCertificate()
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The X.509 certificate is attached to the server options.")]
    public OpenIddictKeyResolversInMemoryBuilder AddDevelopmentSigningCertificate(X500DistinguishedName subject)
    {
        if (subject is null)
        {
            throw new ArgumentNullException(nameof(subject));
    ...
    /// </summary>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddEphemeralSigningKey()
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The X.509 certificate is attached to the server options.")]
    public OpenIddictKeyResolversInMemoryBuilder AddEphemeralSigningKey(string algorithm)
    {
        if (string.IsNullOrEmpty(algorithm))
        {
            throw new ArgumentException(SR.GetResourceString(SR.ID0057), nameof(algorithm));
    ...
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(X509Certificate2 certificate)
    {
        if (certificate is null)
        {
            throw new ArgumentNullException(nameof(certificate));
    ...
    /// <param name="password">The password used to open the certificate.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(Assembly assembly, string resource, string? password)
#if SUPPORTS_EPHEMERAL_KEY_SETS
                // Note: ephemeral key sets are currently not supported on macOS.
    ...
    /// <param name="flags">An enumeration of flags indicating how and where to store the private key of the certificate.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(
        Assembly assembly, string resource,
        string? password, X509KeyStorageFlags flags)
    {
        if (assembly is null)
    ...
    /// <param name="password">The password used to open the certificate.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(Stream stream, string? password)
#if SUPPORTS_EPHEMERAL_KEY_SETS
                // Note: ephemeral key sets are currently not supported on macOS.
    ...
    [SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope",
        Justification = "The X.509 certificate is attached to the server options.")]
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(Stream stream, string? password, X509KeyStorageFlags flags)
    {
        if (stream is null)
        {
            throw new ArgumentNullException(nameof(stream));
    ...
    /// <param name="thumbprint">The thumbprint of the certificate used to identify it in the X.509 store.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(string thumbprint)
    {
        if (string.IsNullOrEmpty(thumbprint))
        {
            throw new ArgumentException(SR.GetResourceString(SR.ID0065), nameof(thumbprint));
    ...
    /// <param name="location">The location of the X.509 store.</param>
    /// <returns>The <see cref="OpenIddictKeyResolversInMemoryBuilder"/>.</returns>
    public OpenIddictKeyResolversInMemoryBuilder AddSigningCertificate(string thumbprint, StoreName name, StoreLocation location)
    {
        if (string.IsNullOrEmpty(thumbprint))
        {
            throw new ArgumentException(SR.GetResourceString(SR.ID0065), nameof(thumbprint));
